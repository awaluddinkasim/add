...
<head>
    ...
    <title>Laporan Pengeluaran Dana</title>
    <style>
        body {
...
<body>
    <div class="header">
        ...
        <h2>Laporan Data Pengeluaran Dana</h2>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            @forelse($pengeluaranDana as $item)
                <tr>
                    <td class="text-center">{{ $no++ }}</td>
                    <td>{{ $item->kegiatan->nama ?? '-' }}</td>
                    <td class="text-center">{{ date('d/m/Y', strtotime($item->tanggal_pengeluaran)) }}</td>
                    <td>{{ $item->pencatat }}</td>
            ...
            @empty
                <tr>
                    <td colspan="6" class="text-center" style="padding: 20px; font-style: italic;">
                        Tidak ada data pengeluaran dana
                    </td>
                </tr>
            @endforelse

            @if ($pengeluaranDana->count() > 0)
                <tr class="total-row">
                    <td colspan="5" class="text-center">TOTAL PENGELUARAN</td>
                    <td class="currency">{{ number_format($totalPengeluaran, 0, ',', '.') }}</td>
                </tr>
            @endif
        </tbody>
    </table>
